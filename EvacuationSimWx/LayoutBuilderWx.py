...
class LayoutBuilderWx():
    ...
    def load_layout_file(self, FORMAT='xlsx'):
        ...
        for row in ws.iter_rows():
            ...
            #newcell = new_ws.cell(row=cell.row, column=cell.column)
                ...
                elif color_str == self.config.LAYOUT_FILE_EMPTY_SPACE_NO_PED:
                    currentCell = LayoutCell(cell.row, cell.column,
                                            self.config.LAYOUT_FILE_EMPTY_SPACE_NO_PED,
                                            self.config.LAYOUT_DISPLAY_EMPTY_SPACE_NO_PED,
        ...
        for row in self.LayoutMap[self.min_row - 1:]:
            for cell in row[self.min_column - 1:]:
                ...
                # for empty cell/empty space, draw grids only (vertical lines/horizontal line)
                    ...
                    if cell.type in(self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE, self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE_NO_PED):
                        ...
                        if cell.type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE:
                            ...
                            # Real empty space. for the No_PED spaces, will not increase this counter
    ...
    def calculatePedCongestion(self):
        ...
        for row in range(len(self.LayoutMap)):
            for column in range(len(self.LayoutMap[row])):
                ...
                or self.LayoutMap[row][column].type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE_NO_PED:
        ...
        for direction in temp_direction_list:
            ...
            if self.rowOrColumnInRange('row', new_row) and self.rowOrColumnInRange('column', new_column):
                if self.LayoutMap[new_row][new_column].type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE \
                    or self.LayoutMap[new_row][new_column].type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE_NO_PED:
    ...
    def calculatePotentialForSubarea(self):
        ...
        ## at this moment we have all potentials for each exit for each evacuation zone.
        ## will assign temp_exit for each evacuation zone based on the e = arg min(F_s_e)
        ## Step 5 in paper for exit choice model
        for item in self.evacuation_zone_info:
            item.temp_exit = min(item.potential_dict, key=item.potential_dict.get)
    ...
    def assignExitForEvacuationZone(self):
        last_potential = 0.0 # f_s_e0
        current_min_potential_exit_id = -1
        ...
        for zone in self.evacuation_zone_info:
            ...
            if zone.preferred_exit != -1: # we haven't assigned exit for current zone yet.
                last_potential = zone.potential_dict[zone.preferred_exit]
                current_min_potential_exit_id = zone.preferred_exit
            else:
                ...
                last_potential = zone.potential_dict[current_min_potential_exit_id]

            # the temporary assigned exit e is different from the optimal exit e0 in the last time
            if zone.temp_exit != zone.preferred_exit:
                # F_s_e0 - F_s_e > θ？
                if last_potential - zone.potential_dict[zone.temp_exit] > self.config.EXIT_CHOICE_MODEL_THETA:
                    logger.info(f"Evacuation Zone : {zone.assigned_zone_id} changed Exit choice from {zone.preferred_exit} to {zone.temp_exit} due to potential difference : {last_potential - zone.potential_dict[zone.temp_exit]}.")
                    zone.preferred_exit = zone.temp_exit
                    # Here we also needs to assign the preferred_exit in the LayoutMap when the preferred exit changed.
                    for i in range(len(self.LayoutMap)):
                        for j in range(len(self.LayoutMap[i])):
                            ...
                            if self.LayoutMap[i][j].assigned_evacuation_zone == zone.assigned_zone_id:
                                self.LayoutMap[i][j].preferred_exit = zone.preferred_exit
            ...
            for j in range(len(self.LayoutMap[i])):
                if self.LayoutMap[i][j].row > self.min_row - 1 and  \
                    ...
                    if self.LayoutMap[i][j].type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE \
                        or self.LayoutMap[i][j].type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE_NO_PED:
    ...
    def calculateDynamicPotential(self, current_LayoutCell, direction, exit_id):
        ...
        if self.rowOrColumnInRange('row', new_row) and self.rowOrColumnInRange('column', new_column):
            ...
            # logger.debug(f"current exit id is {exit_id}")
            ...
            logger.info('Out of Range. Use default dummy potential (Very large potential here to prevent the pedestrian to move towards that direction)')
            return 10000.0
    ...
    def calculateTransitionProbability(self, shuffled_idx_array):
        for idx in shuffled_idx_array:
            ...
            if current_LayoutCell.adjacent_to_exit:
                ...
                ### Updated methodology
                DynamicTransitionProbability_Dict_temp = {}
                for key, value in DynamicPotential_Dict.items():
                    DynamicTransitionProbability_Dict_temp[key] = math.exp(-1 * self.config.PEDESTRIAN_MOVEMENT_MODEL_EPSILON * value) * DynamicCellAccessibility_Dict[key]
                DynamicTransitionProbability_Dict_sum = sum(DynamicTransitionProbability_Dict_temp.values())
                probs = np.array(list(DynamicTransitionProbability_Dict_temp.values()))
                probs = probs / DynamicTransitionProbability_Dict_sum

                for key, new_value in zip(DynamicCellAccessibility_Dict.keys(), probs):
                    DynamicTransitionProbability_Dict[key] = new_value

                max_key = random.choices(list(DynamicTransitionProbability_Dict.keys()), list(DynamicTransitionProbability_Dict.values()))[0]
                max_value = DynamicTransitionProbability_Dict[max_key].item()
                new_row, new_column = self.getNewPosition(current_idx_row, current_idx_column, max_key)
                ...
                if self.rowOrColumnInRange('row', new_row) and self.rowOrColumnInRange('column', new_column):
                    ...
                    logger.debug(f'Pedestrian {idx} move : {max_key} from [row : {current_idx_row + 1}, column : {current_idx_column + 1}] to [row : {new_row + 1}, column : {new_column + 1}] with probability {max_value:.5%} with speed {current_CA.last_speed}')
    ...
    def initializeExitAndEvacuationZone(self):
                    ...
                    for direction in temp_direction_list:
                        ...
                        try:
                            if self.LayoutMap[new_row - 1][new_column - 1].type == self.config.LAYOUT_CELL_TYPE_EXIT:
                                ...
                                # and also assign the exit_id_for_current_zone to current exit_index.
                                for zone in self.evacuation_zone_info:
                                    if zone.assigned_zone_id == self.LayoutMap[i][j].assigned_evacuation_zone:
                                        ...
                                        zone.exit_id_for_current_zone = self.LayoutMap[new_row - 1][new_column - 1].exit_index
                                break
                        except:
        ...
        for row in self.LayoutMap[self.min_row - 1:]:
            for cell in row[self.min_column - 1:]:
                if cell.type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE \
                    or cell.type == self.config.LAYOUT_CELL_TYPE_EMPTY_SPACE_NO_PED:
