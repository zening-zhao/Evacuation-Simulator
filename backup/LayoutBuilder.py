...
class LayoutBuilder():
    ...
    def __init__(self, path, canvas):
        ...
        self.direction_List = [constants.DIRECTION_CENTER,
                                constants.DIRECTION_DOWN,
                                constants.DIRECTION_DOWNLEFT,
                                constants.DIRECTION_DOWNRIGHT,
                                constants.DIRECTION_LEFT,
                                constants.DIRECTION_RIGHT,
                                constants.DIRECTION_UP,
                                constants.DIRECTION_UPLEFT,
                                constants.DIRECTION_UPRIGHT]
    ...
    def load_layout_file(self, FORMAT='xlsx'):
        ...
        for row in ws.iter_rows():
            ...
            #newcell = new_ws.cell(row=cell.row, column=cell.column)
                ...
                if color_str != constants.LAYOUT_FILE_EMPTY_CELL: # first none empty cell of current row
                ...
                if color_str == constants.LAYOUT_FILE_BORDER:
                    currentCell = LayoutCell(cell.row, cell.column,
                                            constants.LAYOUT_NUMBER_BORDER,
                                            constants.LAYOUT_DISPLAY_BORDER,
                                            constants.LAYOUT_CELL_TYPE_BORDER)
                elif color_str == constants.LAYOUT_FILE_OBSTACLE:
                    currentCell = LayoutCell(cell.row, cell.column,
                                            constants.LAYOUT_NUMBER_OBSTACLE,
                                            constants.LAYOUT_DISPLAY_OBSTACLE,
                                            constants.LAYOUT_CELL_TYPE_OBSTACLE)
                elif color_str == constants.LAYOUT_FILE_EXIT:
                    currentCell = LayoutCell(cell.row, cell.column,
                                            constants.LAYOUT_NUMBER_EXIT,
                                            constants.LAYOUT_DISPLAY_EXIT,
                                            constants.LAYOUT_CELL_TYPE_EXIT)
                elif color_str == constants.LAYOUT_FILE_EMPTY_SPACE:
                    currentCell = LayoutCell(cell.row, cell.column,
                                            constants.LAYOUT_NUMBER_EMPTY_SPACE,
                                            constants.LAYOUT_DISPLAY_EMPTY_SPACE,
                                            constants.LAYOUT_CELL_TYPE_EMPTY_SPACE)
                else: ## Default to empty_CELL
                    currentCell = LayoutCell(cell.row, cell.column,
                                            constants.LAYOUT_NUMBER_EMPTY_CELL,
                                            constants.LAYOUT_DISPLAY_EMPTY_CELL,
                                            constants.LAYOUT_CELL_TYPE_EMPTY_CELL)
                ...
                # currentCell.static_potential = {key : -2 for key in range(1, constants.NUMBER_OF_EXIT + 1)}    
    ...
    #---------------------------------------------------------------------------  
    def construct_layoutMap(self, dc):
        ...
        vertical_eva_zone_number = math.ceil((self.max_column - self.min_column - 1) 
                                             / ( constants.EVACUATION_ZONE_DIMENSION / constants.CELL_SIZE))
        ...
        for row in self.LayoutMap[self.min_row - 1:]:
            for cell in row[self.min_column - 1:]:
                if cell.type == constants.LAYOUT_CELL_TYPE_EMPTY_CELL: # skip the empty cell (usually the empty cell is outside of map.)
                    continue
                # for border/obstacle/exit, draw solid rectangle
                if cell.type == constants.LAYOUT_CELL_TYPE_BORDER or \
                    cell.type == constants.LAYOUT_CELL_TYPE_OBSTACLE or \
                    cell.type == constants.LAYOUT_CELL_TYPE_EXIT:
                    gc.SetPen(wx.Pen(wx.Colour('BLACK'), 1, wx.PENSTYLE_TRANSPARENT))
                    gc.SetBrush(wx.Brush(wx.Colour(*ThemeColorConverter.hex_to_rgb(cell.color)),wx.BRUSHSTYLE_SOLID))
                    gc.DrawRectangle(cell.column * constants.LAYOUT_CELL_SIZE,
                                    cell.row * constants.LAYOUT_CELL_SIZE,
                                    constants.LAYOUT_CELL_SIZE,
                                    constants.LAYOUT_CELL_SIZE)
                else:
                # for empty cell/empty space, draw grids only (vertical lines/horizontal line)
                    ...
                    if cell.type == constants.LAYOUT_CELL_TYPE_EMPTY_SPACE:
                        current_assigned_evacuation_zone = int((cell.column - self.min_column - 1) // (constants.EVACUATION_ZONE_DIMENSION /constants.CELL_SIZE) + \
                                                            ((cell.row - self.min_row - 1) // (constants.EVACUATION_ZONE_DIMENSION / constants.CELL_SIZE)) * vertical_eva_zone_number)
                        ...
                        current_ZoneCell.average_speed = constants.NORMAL_PEDESTRIAN_SPEED
                    ...
                    verticle_lines.append((
                                    cell.column * constants.LAYOUT_CELL_SIZE,
                                    cell.row * constants.LAYOUT_CELL_SIZE,
                                    cell.column * constants.LAYOUT_CELL_SIZE + constants.LAYOUT_CELL_SIZE,
                                    cell.row * constants.LAYOUT_CELL_SIZE
                                    ))
                    horizontal_lines.append((
                                    ...
                                    cell.row * constants.LAYOUT_CELL_SIZE,
                                    cell.column * constants.LAYOUT_CELL_SIZE,
                                    cell.row * constants.LAYOUT_CELL_SIZE + constants.LAYOUT_CELL_SIZE
                                    ))
        
        # draw the layout grid
        gc.SetPen(wx.Pen(wx.Colour('GREY'), 1, wx.PENSTYLE_SOLID))
        gc.SetBrush(wx.Brush(wx.Colour(*ThemeColorConverter.hex_to_rgb(cell.color)),wx.BRUSHSTYLE_VERTICAL_HATCH))
        gc.DrawLineList(verticle_lines + horizontal_lines)
        ...
        self.max_pedestrian_capicity = number_of_total_empty_space * constants.CELL_SIZE * constants.PEDESTRIAN_PER_SQUARE_METER
        ...
        self.initPedestrian(number=constants.NUMBER_OF_PEDESTRIAN)      
        ...
        for i in range(len(self.AutomataList)):
            ...
            if self.AutomataList[i].occupied: # only setup the spot that is occupied. (not border/exit/obstacle)
                if self.AutomataList[i].velocity_mode == 2: # senior people
                    EllipseList_Senior.append(((self.AutomataList[i].y) * constants.LAYOUT_CELL_SIZE + 1,
                                                (self.AutomataList[i].x) * constants.LAYOUT_CELL_SIZE + 1,
                                                constants.LAYOUT_CELL_SIZE - 2,
                                                constants.LAYOUT_CELL_SIZE - 2))
                else:
                    EllipseList.append(((self.AutomataList[i].y) * constants.LAYOUT_CELL_SIZE + 1,
                                        (self.AutomataList[i].x) * constants.LAYOUT_CELL_SIZE + 1,
                                        constants.LAYOUT_CELL_SIZE - 2,
                                        constants.LAYOUT_CELL_SIZE - 2))
        gc.SetPen(wx.Pen(wx.Colour(constants.LAYOUT_NON_SENIOR_COLOR), 1, wx.PENSTYLE_SOLID))
        gc.SetBrush(wx.Brush(wx.Colour(constants.LAYOUT_NON_SENIOR_COLOR), wx.BRUSHSTYLE_SOLID))
        gc.DrawEllipseList(EllipseList)
        # debug only
        # print(len(EllipseList))
        gc.SetPen(wx.Pen(wx.Colour(constants.LAYOUT_SENIOR_COLOR), 1, wx.PENSTYLE_SOLID))
        gc.SetBrush(wx.Brush(wx.Colour(constants.LAYOUT_SENIOR_COLOR), wx.BRUSHSTYLE_SOLID))
        gc.DrawEllipseList(EllipseList_Senior)
        # debug only
        # print(len(EllipseList_Senior))

    #---------------------------------------------------------------------------
    ## hardcoded evaculation zone logic?
    ...
    #---------------------------------------------------------------------------  
    def assign_initial_exit_for_zonelist(self):
        for i in range(len(self.evacuation_zone_info)):
            if self.evacuation_zone_info[i].assigned_zone_id == 0:
                self.evacuation_zone_info[i].preferred_exit = 8
            elif self.evacuation_zone_info[i].assigned_zone_id == 1:
                self.evacuation_zone_info[i].preferred_exit = 1
                self.evacuation_zone_info[i].exit_id_for_current_zone = 1
            elif self.evacuation_zone_info[i].assigned_zone_id == 2:
                self.evacuation_zone_info[i].preferred_exit = 1
            elif self.evacuation_zone_info[i].assigned_zone_id == 3:
                self.evacuation_zone_info[i].preferred_exit = 2
                self.evacuation_zone_info[i].exit_id_for_current_zone = 2
            elif self.evacuation_zone_info[i].assigned_zone_id == 4:
                self.evacuation_zone_info[i].preferred_exit = 2
            elif self.evacuation_zone_info[i].assigned_zone_id == 5:
                self.evacuation_zone_info[i].preferred_exit = 8
                self.evacuation_zone_info[i].exit_id_for_current_zone = 8
            elif self.evacuation_zone_info[i].assigned_zone_id == 6:
                self.evacuation_zone_info[i].preferred_exit = 8
            elif self.evacuation_zone_info[i].assigned_zone_id == 7:
                self.evacuation_zone_info[i].preferred_exit = 1
            elif self.evacuation_zone_info[i].assigned_zone_id == 8:
                self.evacuation_zone_info[i].preferred_exit = 2
            elif self.evacuation_zone_info[i].assigned_zone_id == 9:
                self.evacuation_zone_info[i].preferred_exit = 3
                self.evacuation_zone_info[i].exit_id_for_current_zone = 3
            elif self.evacuation_zone_info[i].assigned_zone_id == 10:
                self.evacuation_zone_info[i].preferred_exit = 7
            elif self.evacuation_zone_info[i].assigned_zone_id == 11:
                self.evacuation_zone_info[i].preferred_exit = 7
            elif self.evacuation_zone_info[i].assigned_zone_id == 12:
                self.evacuation_zone_info[i].preferred_exit = 3
            elif self.evacuation_zone_info[i].assigned_zone_id == 13:
                self.evacuation_zone_info[i].preferred_exit = 3
            elif self.evacuation_zone_info[i].assigned_zone_id == 14:
                self.evacuation_zone_info[i].preferred_exit = 3
            elif self.evacuation_zone_info[i].assigned_zone_id == 15:
                self.evacuation_zone_info[i].preferred_exit = 7
                self.evacuation_zone_info[i].exit_id_for_current_zone = 7
            elif self.evacuation_zone_info[i].assigned_zone_id == 16:
                self.evacuation_zone_info[i].preferred_exit = 6
            elif self.evacuation_zone_info[i].assigned_zone_id == 17:
                self.evacuation_zone_info[i].preferred_exit = 5
            elif self.evacuation_zone_info[i].assigned_zone_id == 18:
                self.evacuation_zone_info[i].preferred_exit = 4
            elif self.evacuation_zone_info[i].assigned_zone_id == 19:
                self.evacuation_zone_info[i].preferred_exit = 4
                self.evacuation_zone_info[i].exit_id_for_current_zone = 4
            elif self.evacuation_zone_info[i].assigned_zone_id == 20:
                self.evacuation_zone_info[i].preferred_exit = 6
            elif self.evacuation_zone_info[i].assigned_zone_id == 21:
                self.evacuation_zone_info[i].preferred_exit = 6
                self.evacuation_zone_info[i].exit_id_for_current_zone = 6
            elif self.evacuation_zone_info[i].assigned_zone_id == 22:
                self.evacuation_zone_info[i].preferred_exit = 5
            elif self.evacuation_zone_info[i].assigned_zone_id == 23:
                self.evacuation_zone_info[i].preferred_exit = 5
                self.evacuation_zone_info[i].exit_id_for_current_zone = 5
            elif self.evacuation_zone_info[i].assigned_zone_id == 24:
                self.evacuation_zone_info[i].preferred_exit = 4
            else:
                self.evacuation_zone_info[i].preferred_exit = -1
    ...
    ##  hardcoded evaculation zone logic?
    #---------------------------------------------------------------------------  
    def assign_initial_exit(self, i, j):
        ## TODO this will be a hardcoded logic at this moment. only works for the square layeout.
        ## the algorithm here should be
        # 1. find out the exit from current map and name them sequencely and then loop for all subareas.
        # 2. for the subarea that doesn't directly connect to any exit, assign -1 for their predefined exit. and -1 for 
        #    their distance to nearest exit
        # 3. for the subarea that directly connect to an exit. assign the exit number to this subarea and asssign 0 to 
        #    current subarea's distance
        # 4. start with any subarea and checking all adjacent subarea (Moore neighbour or Von Neumann neighour?) of current 
        #    subarea. put them into a queue and popped out top item and check
        # 5. if that area has exit assigned, then compare it with current area's distance + 1
        # 5. only keep the smaller distance in the area being checked and continue to next item in the top of the queue.
        # 6. stop when all subarea has been checked and all subarea has assigned exit.
        if self.LayoutMap[i][j].assigned_evacuation_zone == 0:
            self.LayoutMap[i][j].preferred_exit = 8
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 1:
            self.LayoutMap[i][j].preferred_exit = 1  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 2:
            self.LayoutMap[i][j].preferred_exit = 1  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 3:
            self.LayoutMap[i][j].preferred_exit = 2  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 4:
            self.LayoutMap[i][j].preferred_exit = 2  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 5:
            self.LayoutMap[i][j].preferred_exit = 8  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 6:
            self.LayoutMap[i][j].preferred_exit = 8  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 7:
            self.LayoutMap[i][j].preferred_exit = 1  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 8:
            self.LayoutMap[i][j].preferred_exit = 2  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 9:
            self.LayoutMap[i][j].preferred_exit = 3  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 10:
            self.LayoutMap[i][j].preferred_exit = 7  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 11:
            self.LayoutMap[i][j].preferred_exit = 7  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 12:
            self.LayoutMap[i][j].preferred_exit = 3  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 13:
            self.LayoutMap[i][j].preferred_exit = 3  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 14:
            self.LayoutMap[i][j].preferred_exit = 3  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 15:
            self.LayoutMap[i][j].preferred_exit = 7  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 16:
            self.LayoutMap[i][j].preferred_exit = 6  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 17:
            self.LayoutMap[i][j].preferred_exit = 5  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 18:
            self.LayoutMap[i][j].preferred_exit = 4  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 19:
            self.LayoutMap[i][j].preferred_exit = 4  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 20:
            self.LayoutMap[i][j].preferred_exit = 6  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 21:
            self.LayoutMap[i][j].preferred_exit = 6  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 22:
            self.LayoutMap[i][j].preferred_exit = 5  
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 23:
            self.LayoutMap[i][j].preferred_exit = 5 
        elif self.LayoutMap[i][j].assigned_evacuation_zone == 24:
            self.LayoutMap[i][j].preferred_exit = 4 
        else:
            self.LayoutMap[i][j].preferred_exit = -1
    ...
    #--------------------------------------------------------------------------- 
    def initPedestrian(self, number=constants.NUMBER_OF_PEDESTRIAN):
        ...
        for i in range(number):
            ...
            try:
                ...
                if np.random.rand() <= constants.SENIOR_PEDESTRIAN_PERCENTAGE:
                    self.AutomataList[rnd_value].velocity_mode = 2 # half speed senior mode
                    self.AutomataList[rnd_value].last_speed = constants.SENIOR_PEDESTRIAN_SPEED
                else:
                    self.AutomataList[rnd_value].velocity_mode = 1 # normal speed mode
                    self.AutomataList[rnd_value].last_speed = constants.NORMAL_PEDESTRIAN_SPEED
            except:
                pass
    ...
    def calculatePedCongestion(self):
        ...
        for row in range(len(self.LayoutMap)):
            for column in range(len(self.LayoutMap[row])):
                ...
                if self.LayoutMap[row][column].type == constants.LAYOUT_CELL_TYPE_EMPTY_SPACE:
    ...
    def getExitIDForCurrentExitCell(self, row, column):
        ...
        temp_direction_list.remove(constants.DIRECTION_CENTER)
        for direction in temp_direction_list:
            ...
            if self.rowOrColumnInRange('row', new_row) and self.rowOrColumnInRange('column', new_column):
                if self.LayoutMap[new_row][new_column].type == constants.LAYOUT_CELL_TYPE_EMPTY_SPACE:
    ...
    def checkAdjacentCongestions(self, row, column, direction):
        ...
        new_row, new_column = self.getNewPosition(row, column, direction)
        if self.LayoutMap[new_row][new_column].type == constants.LAYOUT_CELL_TYPE_BORDER or \
            self.LayoutMap[new_row][new_column].type == constants.LAYOUT_CELL_TYPE_OBSTACLE:
            return 1
    ...
    def calculateStaticPotentialMatrix(self):
        ...
        # for exit_id in range(1, constants.NUMBER_OF_EXIT + 1):
        for exit_id in range(1, len(self.Exit_Dict) + 1):
            ...
            for i in range(len(self.LayoutMap)):
                for j in range(len(self.LayoutMap[i])):
                    ...
                    if self.LayoutMap[i][j].type == constants.LAYOUT_CELL_TYPE_BORDER or \
                        self.LayoutMap[i][j].type == constants.LAYOUT_CELL_TYPE_OBSTACLE:
                    ...
                    if self.LayoutMap[i][j].type == constants.LAYOUT_CELL_TYPE_EXIT:
                        # current exit
            ...
            while(len(potential_list)>0):
                ...
                temp_direction_list.remove(constants.DIRECTION_CENTER)
                for direction in temp_direction_list:
                    if direction in [constants.DIRECTION_DOWN, constants.DIRECTION_UP, constants.DIRECTION_LEFT, constants.DIRECTION_RIGHT]:
                        self.checkAdjacentStaticPotentials(potential_list, current_cell, direction, constants.HORI_VERT_DISTANCE)
                    else:
                        self.checkAdjacentStaticPotentials(potential_list, current_cell, direction, constants.DIAGONAL_DISTANCE)
            ...
            for i in range(len(self.LayoutMap)):
                for j in range(len(self.LayoutMap[i])):
                    # if self.LayoutMap[i][j].type == constants.LAYOUT_CELL_TYPE_EMPTY_SPACE:
    ...
    def calculateAvaragePedestrianSpeed(self):
        ...
        for cell in self.AutomataList:
            if not cell.is_outside and cell.occupied: # check all pedestrian inside of the field only
                ...
                cell.last_speed = (cell.last_step_count * constants.CELL_SIZE) / \
                (constants.SIMULATION_CYCLE * (constants.CELL_SIZE / constants.NORMAL_PEDESTRIAN_SPEED))
        ...
        for exit_id in range(1, len(self.Exit_Dict) + 1):
            ...
            for zone_cell in self.evacuation_zone_info:
                if zone_cell.exit_id_for_current_zone == zone_cell.preferred_exit == exit_id: 
                    ...
                    ped_density = zone_cell.number_of_pedestrian / (constants.EVACUATION_ZONE_DIMENSION * constants.EVACUATION_ZONE_DIMENSION)
                    try:
                        temp_potential = (1 + constants.EXIT_CHOICE_MODEL_BETA * ped_density) * \
                                        (1 + constants.EXIT_CHOICE_MODEL_GAMMA / zone_cell.average_speed)
                    except:
            ...
            while(len(subarea_potential_list)>0):
                ...
                for direction in temp_direction_list:
                    if direction in [constants.DIRECTION_DOWN, constants.DIRECTION_UP, constants.DIRECTION_LEFT, constants.DIRECTION_RIGHT]:    
                        self.checkAdjacentZonePotentials(subarea_potential_list, current_zone_cell, direction, constants.EXIT_CHOICE_MODEL_ALPHA_NON_DIAGONAL)
                    else:
                        self.checkAdjacentZonePotentials(subarea_potential_list, current_zone_cell, direction, constants.EXIT_CHOICE_MODEL_ALPHA_DIAGONAL)
    ...
    def checkAdjacentZonePotentials(self, subarea_potential_list, current_cell, direction, alpha):
        ...
        try:
            if self.rowOrColumnInRangeForZone('row', new_row, self.evacuation_zone_info) and \
                ...
                ped_density = adjacent_zone_cell.number_of_pedestrian / (constants.EVACUATION_ZONE_DIMENSION * constants.EVACUATION_ZONE_DIMENSION)
                try:
                    new_potential = current_cell.temp_potential + \
                                    (1 + alpha) * \
                                    (1 + constants.EXIT_CHOICE_MODEL_BETA * ped_density ) * \
                                    (1 + constants.EXIT_CHOICE_MODEL_GAMMA / adjacent_zone_cell.average_speed)
                except:
                    logger.info("no pedestrain in current zone. will assign default potential!!")
    ...
    def assignExitForEvacuationZone(self):
        for zone in self.evacuation_zone_info:
            ...
            for exit_id, potential in zone.potential_dict.items():
                ...
                if current_min_potential > potential and potential_diff > constants.EXIT_CHOICE_MODEL_THETA:
    ...
    def calculateDynamicPotential(self, current_LayoutCell, direction, exit_id):
        ...
        if self.rowOrColumnInRange('row', new_row) and self.rowOrColumnInRange('column', new_column):
            ...
            return constants.PEDESTRIAN_MOVEMENT_MODEL_DELTA * current_LayoutCell.ped_congestion + \
                    constants.PEDESTRIAN_MOVEMENT_MODEL_PHI * (new_LayoutCell.static_potential[exit_id] - current_LayoutCell.static_potential[exit_id])
        else:
            # out of range cell
            ...
            if current_LayoutCell.adjacent_to_exit:
                ...
                for key, value in DynamicCellAccessibility_Dict.items():
                    DynamicTransitionProbability_Dict[key] = value / \
                        sum(math.exp(-constants.PEDESTRIAN_MOVEMENT_MODEL_EPSILON * 
                ...
                if new_row != current_idx_row or new_column != current_idx_column: # not stay in same position
                    ...
                    if max_key in [constants.DIRECTION_DOWN, constants.DIRECTION_LEFT, constants.DIRECTION_UP, constants.DIRECTION_RIGHT]:
    ...
    def getClosestExitCoordination(self, cell):
        ...
        for direction in temp_direction_list:
            ...
            # if self.rowOrColumnInRange('row', new_row) and self.rowOrColumnInRange('column', new_column):
            if self.LayoutMap[new_row - 1][new_column - 1].type == constants.LAYOUT_CELL_TYPE_EXIT:
                if direction in [constants.DIRECTION_DOWN, constants.DIRECTION_LEFT, constants.DIRECTION_UP, constants.DIRECTION_RIGHT]:
    ...
    def getNewPosition(self, current_row, current_column, direction):
        ...
        if direction == constants.DIRECTION_RIGHT:
            new_row = current_row
            new_column = current_column + 1
        if direction == constants.DIRECTION_DOWNLEFT:
            new_row = current_row + 1
            new_column = current_column - 1
        if direction == constants.DIRECTION_DOWNRIGHT:
            new_row = current_row + 1
            new_column = current_column + 1
        if direction == constants.DIRECTION_UPLEFT:
            new_row = current_row - 1
            new_column = current_column - 1
        if direction == constants.DIRECTION_UPRIGHT:
            new_row = current_row - 1
            new_column = current_column + 1
        if direction == constants.DIRECTION_CENTER:
            new_row = current_row
            new_column = current_column
    ...
    def checkAdjacentCellStatus(self, cell, direction):
        ...
        if self.rowOrColumnInRange('row', new_row) and self.rowOrColumnInRange('column', new_column):
            if self.LayoutMap[new_row][new_column].type in \
                        [constants.LAYOUT_CELL_TYPE_BORDER, 
                            constants.LAYOUT_CELL_TYPE_OBSTACLE] or \
    ...
    def initializeExitAndEvacuationZone(self):
        ...
        for i in range(len(self.LayoutMap)):
            for j in range(len(self.LayoutMap[i])):
                if self.LayoutMap[i][j].type == constants.LAYOUT_CELL_TYPE_EXIT and not self.LayoutMap[i][j].assigned_to_exit_flag:
    ...
    def calculateStaticDistanceMatrixForEvacuationZone(self):   
        ...
        for exit_id in self.Exit_Dict.keys():
            ...
            while(len(zone_list)>0):
                ...
                for direction in temp_direction_list:
                    if direction in [constants.DIRECTION_DOWN, constants.DIRECTION_UP, constants.DIRECTION_LEFT, constants.DIRECTION_RIGHT]:    
                        self.checkAdjacentZoneDistance(zone_list, current_zone_cell, direction, constants.HORI_VERT_DISTANCE)
                    else:
                        self.checkAdjacentZoneDistance(zone_list, current_zone_cell, direction, constants.DIAGONAL_DISTANCE)
            ...
            for direction in self.direction_List:
                ...
                try:
                    if self.LayoutMap[new_row - 1][new_column - 1] is not None: # if we can access the cell in current direction
                        if self.LayoutMap[new_row - 1][new_column - 1].type == constants.LAYOUT_CELL_TYPE_EXIT and \
